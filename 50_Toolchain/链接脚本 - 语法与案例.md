...
---
> [!abstract] 使用方式
> 本文保留详细语法和案例；概念性学习入口参见 [[链接脚本 - 核心概念]]。
...
![[assets/50_Toolchain/compilation_pipeline.jpg]]
...
为一个做工程创建一个**符号表**，里面会记录每个文件定义的全局符号、每个文件所引用的外部符号、每个文件的内部定义符号。类似于 Source Insight中的符号表
